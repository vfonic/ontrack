...
class ColorPicker extends React.Component {
	...
	colors() {
		...
		for (let color of allColors) {
			if (this.props.colorsToShow && this.props.colorsToShow === availColors.length) {
				break
			}
			if (
				availColors
			...
			if (
				this.props.omitColors.length &&
				this.props.omitColors
					.map(el => {
						return el.toLowerCase()
					})
					.includes(color.toLowerCase())
	...
	render() {
		return <div className="color-picker">{this.renderSwatches()}</div>
	}
}
