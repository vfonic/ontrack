...
class Paginator extends React.Component {
	...
	handlePageNext = () => {
		if (this.state.selectedPage + 1 > this.state.totalPages) {
			return
		}
	...
	displayedPages() {
		...
		if (this.state.totalPages <= 10) {
			...
			const nextSlidingLowerBound =
				this.state.selectedPage >= this.state.totalPages - 4 ? this.state.totalPages - 9 : this.state.selectedPage - 5
			...
			const nextSlidingUpperBound =
				this.state.selectedPage + 4 > this.state.totalPages ? this.state.totalPages : this.state.selectedPage + 4
		...
		return (
			<div className="paginator">
				<div className="pagination-num-items">
					<span>
						Showing <strong>{this.itemsLowerLimit()}</strong>-<strong>{this.itemsUpperLimit()}</strong> of{" "}
				...
				<ol>
					<li>
						<a onClick={this.handlePagePrev}>
							<i className="fa fa-angle-left"></i>
						</a>
					</li>
					{this.displayedPages().map(p => {
						return this.renderPage(p)
					})}
					<li>
						<a onClick={this.handlePageNext}>
