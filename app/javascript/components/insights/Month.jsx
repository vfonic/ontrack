...
class Year extends React.Component {
	...
	handleMonthDecrement = () => {
		this.setState(
			{ month: this.props.availableMonths[this.props.availableMonths.indexOf(this.state.month) - 1] },
			this.loadData
		)
	}
		...
		new Chart(document.getElementById("chart2"), {
			...
			options: {
				...
				tooltips: {
					callbacks: {
						label: t => {
							return `${this.state.chartData.datasets[t.datasetIndex].label}: $${Numerics.commify(
								parseFloat(t.yLabel).toFixed(2)
							)}`
						},
					},
				},
	...
	renderTotalGoal() {
		...
		return (
			<div className="month flex flex-space-between">
				<b>Goal comparison</b>
				<h2 className={amountOver <= 0 ? "text-success" : "text-warning"}>
					{amountOver >= 0 ? "+" : "-"}
					{Numerics.centsToDollars(Math.abs(amountOver))}
				</h2>
			</div>
		)
	}
	...
	render() {
		return (
			<div>
				<div>
					{this.props.availableMonths.indexOf(this.state.month) == 0 ? (
						""
					) : (
						<a onClick={this.handleMonthDecrement}>
							<i className="fa fa-chevron-left mr-10"></i>
						</a>
					)}
					{this.state.month}
					{this.props.availableMonths.indexOf(this.state.month) == this.props.availableMonths.length - 1 ? (
						""
					) : (
						<a onClick={this.handleMonthIncrement}>
