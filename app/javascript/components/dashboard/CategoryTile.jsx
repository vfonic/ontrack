...
class CategoryTile extends React.Component {
	...
	openCategoryEdit = () => {
		this.setState({ showCategoryEditModal: true })
	}
	closeCategoryEdit = () => {
		this.setState({ showCategoryEditModal: false })
	}
	openExpenseCreate = () => {
		this.setState({ showExpenseCreateModal: true })
	}
	closeExpenseCreate = () => {
		this.setState({ showExpenseCreateModal: false })
	}
	onCategorySave = () => {
	...
	normalizedPercentage() {
		...
		return Math.min(
			100,
			(this.props.categoryWithExpensesAndSpend.spend / this.props.categoryWithExpensesAndSpend.monthly_goal) * 100
		)
	}
	...
	render() {
		return (
			<div
				...
				style={{ borderColor: this.props.categoryWithExpensesAndSpend.color }}
				onClick={this.jumpToHistory}
			>
				...
				<div className="flex flex-space-between title">
					<div>
						<div className="flex">
							<a
								href={`/expenses?category_id=${
									this.props.categoryWithExpensesAndSpend.id
								}&paid_after=${moment().startOf("month").unix()}`}
								className="dim-on-hover"
							>
								<b>{this.props.categoryWithExpensesAndSpend.name}</b>
							</a>
							<img
								className="icon-default hover-pointer dim-til-hover"
								src={window.iconEdit}
