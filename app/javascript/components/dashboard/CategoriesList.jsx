...
class CategoriesList extends React.Component {
	...
	colorsToSkip() {
		return this.props.categoriesWithExpensesAndSpend.map(cat => {
			return cat.color
		})
	}
	...
	renderCategory(category, idx) {
		...
		return (
			<div className="six columns" key={idx}>
				{markup}
			</div>
		)
	}
	...
	renderRow(listChunk, idx) {
		return (
			<div className="row" key={idx}>
				{listChunk.map((category, idx) => {
					return this.renderCategory(category, idx)
				})}
			</div>
		)
	}
	...
	render() {
		return (
			<div className="clearfix">
				...
				{this.chunkedCategories().map((listChunk, idx) => {
					return this.renderRow(listChunk, idx)
				})}
			</div>
		)
	}
